...
3. Inspect existing mappings with `tailscale serve status`. Do not overwrite or reset unrelated routes.
4. Bind the application to `127.0.0.1`, verify it locally, and expose HTTP with `tailscale serve --bg --https=<port> http://127.0.0.1:<port>`.
5. Verify the service through its local and tailnet addresses, then run `tailscale serve status`. Never invent a hostname.
6. For Docker, publish only to loopback, for example `127.0.0.1:5432:5432`. For raw TCP, verify locally and run `tailscale serve --bg --tcp=<port> tcp://127.0.0.1:<port>`.
7. Report the process, local port, remote address, verification result, and the exact project-scoped command or process action that stops what this task created.
