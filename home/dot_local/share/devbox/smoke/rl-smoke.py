...
def main():
    ...
    path='/checkpoints/rl-smoke.pt'; torch.save({'tensor':y[0,:16].cpu(),'version':torch.__version__},path); restored=torch.load(path,weights_only=True); assert restored['tensor'].shape[0]==16
